...
- **Window aesthetic:** sharp corners (`border-radius: 0`), 2px solid borders, transparent backgrounds — the shell pills (three top pills + bottom workspace pill) are the one rounded exception. Each module renders its own embedded title bar (icon + name, `.module-title`) — the WM does not draw module titles.
