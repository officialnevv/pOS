...
- **Left:** Project name, "pOS"
- **Center:** Current date (live, updates automatically)
- **Right:** Current time (live, updates automatically) in **12-hour format, minutes only** (e.g. "2:45 PM", no seconds); the exact time *with* seconds is shown only in a hover tooltip on the time
...
- **Empty workspace state:** when a workspace has zero windows open, show a subtle centered hint text (e.g. "Alt+Enter to launch a module"), with the keybind portion rendered as a small bordered `<kbd>`-style box rather than plain inline text. Nothing else renders.
...
- Two additional icons live in this pill, **flanking the workspace numbers** — one icon on each side, numbers in the middle (e.g. layout toggle left, theme toggle right):
...
### Dwindle (default for new workspaces)
...
- Shows:
  - The name **"pOS"**, pre-filled and **not editable** (just a display label, not a username field).
  - A PIN input field.
- **PIN:** `464466`, hardcoded in the client-side JS.
- **Auto-unlock:** the moment the correct PIN is fully entered, the overlay unlocks — no Enter key press required. On success the overlay **fades out** (CSS opacity transition) rather than disappearing instantly.
